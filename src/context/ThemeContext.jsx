...
export const ThemeProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    root.style.setProperty(
      "--color-bg-elevated",
      isDark ? darkBgElevatedOverride : colorBgElevated
    );
    root.style.setProperty(
      "--color-bg-container",
      isDark ? darkBgContainerOverride : colorBgContainer
    );
    root.style.setProperty(
      "--color-bg-base",
      isDark ? darkBgBase : colorBgBase
    );
    root.style.setProperty(
      "--color-bg-body",
      isDark ? darkBgBase : colorBgBase
    ...
    root.style.setProperty(
      "--color-text-primary",
      isDark ? darkTextOverride : colorTextBase
    );
    ...
    // Dark Mode Shadow
    const shadowColor =
      themeMode === "dark" ? "rgba(0, 0, 0, 0.4)" : "rgba(0, 0, 0, 0.1)";
    root.style.setProperty("--color-shadow", shadowColor);

    // LOGIC CHO PREMIUM CTA BOX
    const premiumBg =
      themeMode === "dark"
        ? `linear-gradient(135deg, ${colorPrimary}, ${
            colorPrimaryDark || colorPrimary
          })`
